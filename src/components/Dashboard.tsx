...
export default function Dashboard({ child }: { child: Child }) {
  ...
  const { rows, hasWeighIn, wet24, dirty24 } = data;
  const today = rows[rows.length - 1];
  const chart = rows.slice(-30).map((r) => ({ ...r, day: r.date.slice(5).replace('-', '/') }));

  // Formula share over three windows: today, rolling 7 days, all-time.
  ...
  const fToday = pct(today.formulaMl, today.totalEstMl);
  ...
  return (
    <div className="space-y-4 pt-2">
      <div className="grid grid-cols-2 gap-2">
        ...
        <Kpi label="Formula % (today)"
          value={fToday === null ? '—' : `${fToday}%`}
